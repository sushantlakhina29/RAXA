...
<br>
    <center><h2>LICENCE DETAIL
    </h2>
     <br>
    <form name="ff" action="licence.jsp">
        ENTER NAME
 ...
 ResultSet rs=st.executeQuery("select * from licenced where licenceno like '" + s1 +  "' and name like '%" + s + "%' order by licenceno");
        
        out.print("<center><table border=1 bgcolor=pink>");
               
               out.print("<th>LICENCE NO<TH>NAME<TH>FATHER NAME<TH>DATE OF VALIDITY<TH>BLOOD GROUP<TH>ADDRESS");
            while(rs.next())
            {
          
                   out.print("<tr>");
                   
                    out.print("<td>" + rs.getString(1)+"<td>" + rs.getString(2)+"<td>" + rs.getString(3)+"<td>" + rs.getString(4) + "<td>" +rs.getString(5) + "<td>" +rs.getString(6));          
                    t++;
            }
